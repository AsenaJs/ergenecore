...
describe('static file 404 goes through the application onNotFound', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'asena-static-'));
  ...
  test('the per-route file hook can still take over with override', async () => {
    const baseUrl = await boot({ withHook: true, fileHookOverride: true });
    const response = await fetch(`${baseUrl}/assets/nope.txt`);

    // override: true means "I handled it, run the route handler" - the config hook must not
    // pre-empt that.
    expect(response.status).toBe(200);
