...
import { AsenaWebSocketService, BunLocalTransport } from '@asenajs/asena/web-socket';
...
describe('socket.publish() self-delivery does not depend on the transport', () => {
  ...
  async function boot(namespace: string, transport?: WebSocketTransport) {
  ...
  it('warns once at startup when the transport has no publishRemote', async () => {
    ...
    const warned = (mockLogger.warn as any).mock.calls.some((call: any[]) => String(call[0]).includes('publishRemote'));
